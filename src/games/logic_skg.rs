...
pub fn check_weight(append_next: &mut bool, weight: String, c: char) -> Option<String> {
    let c = c.to_lowercase();
    if *append_next {
        *append_next = false;
        Some(format!("Sg_{}{}.png", weight, c))
    } else {
...
pub fn translate_inputs(
    ...
    let temp_input = match c.to_ascii_uppercase() {
        ...
        'P' => check_weight(append_next, weight.clone().unwrap(), c),
        'K' => check_weight(append_next, weight.clone().unwrap(), c),
